 ...
 * @Date: 2020-12-15 16:11:30
 * @LastEditors: whyour
 * @LastEditTime: 2020-12-15 16:27:09
 */
const $ = new Env('50豆');
...
function doShopTask() {
  return new Promise(resolve => {
    $.post(taskPostUrl('doTask', `groupType=FOLLOW_SHOP&itemId=1000096004`), async (err, resp, _data) => {
      try {
				$.log(`开始关注店铺任务`);
...
function doChannelTask() {
  return new Promise(resolve => {
    $.post(taskPostUrl('doTask', `groupType=FOLLOW_CHANNEL&itemId=118`), async (err, resp, _data) => {
      try {
				$.log(`开始关注频道任务`);
        const { success } = JSON.parse(_data);
        $.log(`\n${success}\n${_data}`);
...
function doCartTask() {
  return new Promise(resolve => {
    $.post(taskPostUrl('doTask', `groupType=ADD_CART&itemId=10024095249086`), async (err, resp, _data) => {
      try {
				$.log(`开始加购商品任务`);
        const { success } = JSON.parse(_data);
        $.log(`\n${success}\n${_data}`);
...
function taskPostUrl(function_path, body = {}) {
  return {
    url: `${JD_API_HOST}module/task/${function_path}?configCode=cb0880b159574f0ab3400be8fe4fc484&finishCount=2&${body}&eid=REXKFWQQ3KBANI54JV5ADW44HP4VTOURHDQ6CR2BV3XIFHC4VZWXMDGJJHLDS6PIQJODYFKU2KIRXDAYSO5XGCZGOQ&fp=d8bd3f362e5c23bbbc74fea9a7d4c3e2`,
    headers: {
