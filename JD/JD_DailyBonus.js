...
var Key = 'pt_key=AAJfEQuHADCMWJvicZUI5TfbrPucDe0tL042ToRdS_nbKSED90ROOBP2N7lhkHtVeFjHaJzxpfs;pt_pin=jd_5684bc2e3bc8d;'; //单引号内自行填写您抓取的Cookie

var DualKey = 'pt_key=AAJfFUALADCviUQWHI0rpntwbJA0sEI3Qq1eyfyWyB9OyYcOQrfwuDZisx883p19SHZJqQR8OUQ;pt_pin=%E5%BF%83%E6%99%B4412;'; //如需双账号签到,此处单引号内填写抓取的"账号2"Cookie, 否则请勿填写

var ThreeKey = 'pt_key=AAJfFT6eADDdQPFRocs995k2rtzPXJIQt9ip0F8PVZ7b8AcWGry3B6aZX9-FgLOUp1lQVrTG0mk;pt_pin=jd_EYAxYHyCPNtN;'; //如需三账号签到,此处单引号内填写抓取的"账号3"Cookie, 否则请勿填写
...
function notify() {
  return new Promise(resolve => {
    try {
      ...
      if ($nobyda.isJSBox) {
        if (add && DualAccount && addThree && ThreeAccount) {
          Shortcut = Name + one + two + three + "\n"
        } else if (!add && DualAccount && !addThree && ThreeAccount ) {
...
function ReadCookie() {
  ...
  if ($nobyda.isJSBox) {
    ...
    addThree = ThreeKey || $file.exists("shared://JD_Cookie3.txt") ? true : false
    if (ThreeKey) {
      $file.write({
        data: $data({
          string: ThreeKey
    ...
    addThree = ThreeKey || $nobyda.read("CookieJD3") ? true : false
...
function triple() {
  ...
  ThreeAccount = false
  if ($nobyda.isJSBox) {
    if (ThreeKey || $file.exists("shared://JD_Cookie3.txt")) {
      KEY = ThreeKey ? ThreeKey : $file.read("shared://JD_Cookie3.txt").string
      all()
    } else {
      $nobyda.time();
    }
  } else if (ThreeKey || $nobyda.read("CookieJD3")) {
    KEY = ThreeKey ? ThreeKey : $nobyda.read("CookieJD3")
    all()
  } else {
    $nobyda.time();
    $nobyda.done();
  ...
  try {
    if ($request.headers && $request.url.match(/api\.m\.jd\.com.*=signBean/)) {
      ...
      if (CV.match(/(pt_key=.+?pt_pin=|pt_pin=.+?pt_key=)/)) {
        ...
        } else if (!AccountThree || UserName == AccountThree) {
          var CookieName = " [账号三] ";
